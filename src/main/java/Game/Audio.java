...
public class Audio {
    ...
    public static void resetAndStart(Sound s) {
        Clip c = clips.get(s);
        c.setMicrosecondPosition(0);
        c.start();
    }

    public static void start(Sound s) {
        clips.get(s).start();
    }
    ...
    static void init() throws Exception {
        AudioInputStream stream = AudioSystem.getAudioInputStream(new File("res/audio/spk.wav"));
        ...
        stream = AudioSystem.getAudioInputStream(new File("./res/audio/Hellwalker.wav"));
        clip = AudioSystem.getClip();
        clip.open(stream);
        clip.loop(Clip.LOOP_CONTINUOUSLY);
        clip.stop();
